...
const form = reactive({ name: '', email: '', phone: '', start_date: '', end_date: '', participants: 2, message: '' })
...
watch(isOpen, (value) => {
  if (value) {
    ...
    Object.assign(form, { name: '', email: '', phone: '', start_date: '', end_date: '', participants: 2, message: '' })
    reference.value = ''
    errorMessage.value = ''
  }
})
        ...
        <aside class="booking-drawer">
          ...
          <form v-else @submit.prevent="submit">
            ...
            <div class="form-row">
              ...
              <label v-if="isAccommodation"><span>{{ t('forms.checkOut') }}</span><div class="input-icon"><CalendarDays :size="17" /><DatePicker v-model="form.end_date" /></div></label>
              <label v-else><span>{{ selected?.type === 'transport' ? t('forms.people') : t('forms.guests') }}</span><div class="guest-input"><button type="button" aria-label="Decrease guests" @click="form.participants = Math.max(1, form.participants - 1)"><Minus :size="15" /></button><strong>{{ form.participants }}</strong><button type="button" aria-label="Increase guests" @click="form.participants = Math.min(100, form.participants + 1)"><Plus :size="15" /></button></div></label>
            </div>
            <label v-if="isAccommodation"><span>{{ t('forms.guests') }}</span><div class="guest-input"><button type="button" aria-label="Decrease guests" @click="form.participants = Math.max(1, form.participants - 1)"><Minus :size="15" /></button><strong>{{ form.participants }}</strong><button type="button" aria-label="Increase guests" @click="form.participants = Math.min(100, form.participants + 1)"><Plus :size="15" /></button></div></label>
